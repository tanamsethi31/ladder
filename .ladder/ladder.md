...
- [x] **R021** — Ambiguous pronoun references to rungs are unreliable → *medium*
  ...
  - Note: Correction: the SKILL.md Rule 12 fix above was NOT actually verified by that test - checked with --verbose after the fact and the Skill tool was never invoked in that run, so Rule 12's text was never in context; the model just reasoned its way to the right answer on that toy 2-rung fixture regardless of the rule. Found this by dogfooding for real against footymodel's actual ladder (much richer, 10 rungs): asked 'why haven't we decided on the market thing yet' (genuinely ambiguous between R003 and R010 there) and it answered by reading the file directly and picking one silently, invoking the Skill tool only afterward - too late to matter. Root cause: rules living only in SKILL.md apply only if the Skill tool happens to get invoked before the model commits to an answer, which is a probabilistic judgment call, not guaranteed - SessionStart only ever injected a terse pointer at the skill, never the rule content itself. Real fix: moved the specific disambiguation instruction into session_start.py's injected context directly, so it's guaranteed present before any answer forms, regardless of whether the skill gets invoked. Re-verified against all three fixtures with --verbose confirming actual mechanism, not just output: real footymodel case now correctly asks (previously failed), synthetic ambiguous case still correctly asks, synthetic unambiguous case still answers directly without asking. This is the second time in this project a 'proven' fix turned out to be a false positive from an insufficiently adversarial test (R008 was the first) - the lesson is to check the actual tool-call trace, not just final text output, especially once a fixture is complex enough that plain reasoning could get the right answer for the wrong reason.
